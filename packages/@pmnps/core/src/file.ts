...
function readFile(locationPath: string): Promise<string> {
  return new Promise((resolve, reject) => {
...
async function rename(sourcePath:string,targetPath:string):Promise<boolean>{
  return new Promise((resolve, reject)=>{
...
async function createFileIntoDirIfNotExist(
  ...
  const allNotExist = (ends || []).every(
    end => !fs.existsSync(join(dirPath, `${name}.${end}`))
  );
  if (!fs.existsSync(join(dirPath, filename)) && allNotExist) {
    await createFile(join(dirPath, filename));
  }
}
